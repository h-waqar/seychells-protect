class SwiftNavigation {
    ...
    init() {
        ...
        this.btn_GroupPassportResponseContinue = this.$('#btn_GroupPassportResponseContinue').get(0);
        this.btn_ProcessingPeriodContinue = this.$('#btn_ProcessingPeriodContinue').get(0);
        this.btn_MedicalProtectionContinue = this.$('#btn_MedicalProtectionContinue').get(0);
        this.sb_ProcessingPeriod = this.$('#sb_ProcessingPeriod').get(0);
        this.sb_MedicalProtection = this.$('#sb_MedicalProtection').get(0);
        this.sb_CompleteYourApplication = this.$('#sb_CompleteYourApplication').get(0);
        ...
        if (_visaSwift.sb_PassportInformation) {
            _visaSwift.sb_PassportInformation.addEventListener('click', this.passport.bind(this));
        }

        if (_visaSwift.sb_SelfiePhoto) {
            _visaSwift.sb_SelfiePhoto.addEventListener('click', this.selfie.bind(this));
        }

        if (_visaSwift.sb_ContactInformation) {
            _visaSwift.sb_ContactInformation.addEventListener('click', this.contactInfo.bind(this));
        }

        if (_visaSwift.btn_PaymentOptionPrev) {
            _visaSwift.btn_PaymentOptionPrev.addEventListener('click', this.handle_PrevPaymentOptions.bind(this));
        }

        if (_visaSwift.btn_ProceedPrev) {
            _visaSwift.btn_ProceedPrev.addEventListener('click', this.handle_btnProceedPrev.bind(this));
        }

        if (_visaSwift.btn_OptionalDocsPrev) {
            _visaSwift.btn_OptionalDocsPrev.addEventListener('click', this.handle_btnOptionalDocsPrev.bind(this));
        }

        if (_visaSwift.btn_CustomsInfoPrev) {
            _visaSwift.btn_CustomsInfoPrev.addEventListener('click', this.handle_btnCustomsInfoPrev.bind(this));
        }

        if (_visaSwift.btn_RequiredDocsPrev) {
            _visaSwift.btn_RequiredDocsPrev.addEventListener('click', this.handle_btnRequiredDocsPrev.bind(this));
        }

        if (_visaSwift.btn_HealthInfoPrev) {
            _visaSwift.btn_HealthInfoPrev.addEventListener('click', this.handle_btnHealthInfoPrev.bind(this));
        }

        if (_visaSwift.btn_TripInfoPrev) {
            _visaSwift.btn_TripInfoPrev.addEventListener('click', this.handle_btnTripInfoPrev.bind(this));
        }

        if (_visaSwift.btn_ContactInfoPrev) {
            _visaSwift.btn_ContactInfoPrev.addEventListener('click', this.handle_btnContactInfoPrev.bind(this));
        }

        if (_visaSwift.btn_SelfieResponsePrev) {
            _visaSwift.btn_SelfieResponsePrev.addEventListener('click', this.handle_btnSelfieResponsePrev.bind(this));
        }

        if (_visaSwift.btn_SelfiePhotoPrev) {
            _visaSwift.btn_SelfiePhotoPrev.addEventListener('click', this.handle_btnSelfiePhotoPrev.bind(this));
        }
        ...
        if (_visaSwift.sb_TripInformation) {
            _visaSwift.sb_TripInformation.addEventListener('click', this.handle_sbTripInformation.bind(this));
        }

        if (_visaSwift.sb_HealthDeclaration) {
            _visaSwift.sb_HealthDeclaration.addEventListener('click', this.handle_btnCustomsInfoPrev.bind(this));
        }

        if (this.btn_GroupPassportResponseContinue) {
            this.btn_GroupPassportResponseContinue.addEventListener('click', this.contactInfo.bind(this));
        }
        ...
        if (this.btn_ProcessingPeriodContinue) {
            this.btn_ProcessingPeriodContinue.addEventListener('click', this.handle_btnProcessingPeriodContinue.bind(this));
        }

        if (this.btn_MedicalProtectionContinue) {
            this.btn_MedicalProtectionContinue.addEventListener('click', this.handle_btnMedicalProtectionContinue.bind(this));
        }


        if (this.sb_ProcessingPeriod) {
            this.sb_ProcessingPeriod.addEventListener('click', this.moveToProcessingPeriod.bind(this));
        }


        if (this.sb_MedicalProtection) {
            this.sb_MedicalProtection.addEventListener('click', this.handle_btnProcessingPeriodContinue.bind(this));
        }


        if (this.sb_CompleteYourApplication) {
            this.sb_CompleteYourApplication.addEventListener('click', this.handle_btnMedicalProtectionContinue.bind(this));
        }
        ...
        // 
        this.btn_GroupPassportInfoPrev = this.$('#btn_GroupPassportInfoPrev').get(0);


        if (this.btn_GroupPassportInfoPrev) {
            this.btn_GroupPassportInfoPrev.addEventListener('click', this.handle_btnGroupPassportInfoPrev.bind(this));
        }
    ...
    handle_btnProcessingPeriodContinue(event) {
        ...
        _swiftStorage.processingPeriod = this.$('#processingPeriod input[name="cs_processing_period"]:checked').val();
    ...
    handle_btnSelfieResponsePrev(event) {
        ...
        _visaSwift.showTitle('Selfie or Photo', 'Please upload a selfie or passport-type photo.');
    }
    ...
    handle_btnCustomsInfoPrev(event) {
        ...
        _visaSwift.showTitle('Health Declaration', 'Answer a few questions related to your current health.');
    }
    ...
    handle_btnOptionalDocsPrev(event) {
        ...
        _visaSwift.showTitle('Required Documents', 'Please upload the required document(s) below.');
    }
    ...
    handle_btnTripInfoPrev(e) {
        ...
        if (_visaSwift._applicationSingle) {
            ...
            // Handle Previous For Group //pending77
        }
    }
    ...
    move_NextFromPassportPage() {
        ...
        if (!_visaSwift._applicationSingle) {
            ...
            if (!_visaSwift._applicationSingle) this.$('#btn_PResAddPassport').removeClass('hidden');
