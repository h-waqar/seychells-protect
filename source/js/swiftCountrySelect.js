...
class swiftCountrySelect {
    ...
    constructor($, element, single = true, number) {
        ...
        this.single = single;
        this.number = number;
        this.csCountryContainer = this.$("#" + this.element).find(".cs-country-container");

        this.addHTML();
        this.init();


        this.$("#multiCountryWrap" + this.number).on("click", ".deselect_country", this.deselect_country.bind(this));
    ...
    deselect_country(e) {
        ...
        var selectedCountryDiv = this.$('.sel_country [data-country="' + country + '"]');
    ...
    removeCountry(documentNumber, countryName) {
        ...
        if (_swiftStorage.passports[documentNumber] && _swiftStorage.passports[documentNumber].health_countries) {
            const healthCountries = _swiftStorage.passports[documentNumber].health_countries;
    ...
    isCountryExists(countryName, parentId) {
        ...
        this.$('#' + parentId + ' .sel_country').each(function () {
            ...
            if (_visaSwift._applicationSingle)
                $countryElement = _swiftCS.$(this);
            else
                $countryElement = _swiftGroupCS.$(this);



            var dataCountry = $countryElement.find('i.deselect_country').data('country');
  ...
  `;
            ...
            if (_visaSwift._applicationSingle)
                ...
                this.$("#countriesList2").append(countryDiv);
    ...
    init() {
        ...
        this.searchInput = this.$(this.csCountryContainer).find("#searchInput" + this.number);
        this.countriesList = this.$(this.csCountryContainer).find("#countriesList" + this.number);
        this.selectedCountry = this.$(this.csCountryContainer).find("#selectedCountry" + this.number);
        ...
        this.$(this.searchInput).on('input', () => {
            ...
            if (_visaSwift._applicationSingle)
                ...
                searchTerm = _swiftGroupCS.$(_swiftGroupCS.searchInput).val().trim();
            ...
            if (selectedCountryDiv) {
                ...
                if (selectedCountryData) {
                    if (_visaSwift._applicationSingle)
                        ...
                        _swiftGroupCS.displaySelectedCountry(selectedCountryData);
...
jQuery(document).ready(() => {
    //unitTesting
    window._swiftCS = new swiftCountrySelect(jQuery, "five_HealthDeclaration", false, 1);
});
